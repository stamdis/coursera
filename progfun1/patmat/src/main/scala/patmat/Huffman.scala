...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = chars match {
    case Nil => Nil
    case char :: Nil => List((char, 1))
    case char :: tail => {
      val alreadyCalculated = times(tail)
      if (alreadyCalculated.map(_._1).contains(char))
        alreadyCalculated.map(pair => if (pair._1 == char) (pair._1, pair._2 + 1) else pair)
      else
        (char, 1) :: alreadyCalculated
    }
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case tree1 :: tree2 :: tail => (makeCodeTree(tree1, tree2) :: tail).sortBy(weight(_))
    case _ => trees
  }
   ...
   */
  def until(shouldStop: List[CodeTree] => Boolean,
            execute: List[CodeTree] => List[CodeTree])(input: List[CodeTree]): CodeTree = {
    if (shouldStop(input))
      input.head
    else
      until(shouldStop, execute)(execute(input))
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def traverse(subtree: CodeTree)(char: Char): List[Bit] = subtree match {
      ...
      case Fork(left, right, _, _) =>
        if (chars(left).contains(char)) 0 :: traverse(left)(char) else 1 :: traverse(right)(char)
    }
