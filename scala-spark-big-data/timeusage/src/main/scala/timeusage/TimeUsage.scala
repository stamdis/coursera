...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryNeedsNames = List("t01", "t03", "t11", "t1801", "t1803")
    val workNames = List("t05", "t1805")
    val otherNames = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")
    columnNames.foldRight[(List[Column], List[Column], List[Column])](Nil, Nil, Nil){
      case (name, (primaryNeeds, work, other)) =>
        if (primaryNeedsNames.exists(name.startsWith(_)))
          (col(name) :: primaryNeeds, work, other)
        else if (workNames.exists(name.startsWith(_)))
          (primaryNeeds, col(name) :: work, other)
        else if (otherNames.exists(name.startsWith(_)))
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    // getAs does not maintain the nullable settings of the schema plus results in more lines of code
