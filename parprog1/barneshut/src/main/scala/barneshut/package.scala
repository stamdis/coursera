...
package object barneshut {
  ...
  extends Quad {
    val ma = bodies.foldLeft(0.0f)((acc, body) => acc + body.mass)
    val mx = bodies.foldLeft(0.0f)((acc, body) => acc + body.mass * body.x) / ma
    val my = bodies.foldLeft(0.0f)((acc, body) => acc + body.mass * body.y) / ma
    val (mass, massX, massY) = (ma : Float, mx : Float, my : Float)
    ...
    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        ...
        val newSite = size / 2
        val newFork = Fork(
          Empty(centerX - centerOffset, centerY - centerOffset, newSite),
          Empty(centerX + centerOffset, centerY - centerOffset, newSite),
          Empty(centerX - centerOffset, centerY + centerOffset, newSite),
          Empty(centerX + centerOffset, centerY + centerOffset, newSite)
        )
        (bodies :+ b).foldLeft(newFork)((forkAcc, body) => forkAcc.insert(body))
      } else
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      apply(
        (((b.x - boundaries.minX) / sectorSize).max(0).min(sectorPrecision - 1)).toInt,
        (((b.y - boundaries.minY) / sectorSize).max(0).min(sectorPrecision - 1)).toInt
      ) +=  b
      this
    }
